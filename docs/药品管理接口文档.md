...
| └─ parentCategoryId | long | 否 | 大类ID（通过关联表查询其子分类下的药品） | 2001 |
...
| data | object | 响应数据 | - |
| └─ id | long | 关联ID | 80001 |
| └─ medicineId | long | 药品ID | 50001 |
| └─ categoryId | long | 分类ID | 3011 |
| └─ createdAt | string | 创建时间 | "2025-12-24 08:42:30" |
| └─ medicineName | string | 药品名称 | "八珍颗粒" |
| └─ categoryName | string | 分类名称 | "中药调理" |
...
- 药品信息管理支持根据大类ID查询（parentCategoryId参数），查询该大类下的子分类（小类）关联的药品
- 药品信息管理返回值中包含小类名称（subCategoryName），方便前端显示
- 药品分类管理支持层级查询（parentId参数），查询指定父分类下的子分类